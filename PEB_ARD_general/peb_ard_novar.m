...
%       .lambda_floor  (1e-9)
%       .lambda_ceil   (1e9)
%       .jitter0       (1e-9)   % (reserved for future)
%
% Outputs (key):
%   M.beta         : p_kept×d posterior means (original scale, kept cols only)
%   M.beta_std     : p_kept×d posterior means (standardised space)
...
%   M.z, M.pvals   : Wald stats (std space)
%   M.kept_columns : indices mapping kept cols -> original X columns
%   M.x_mean/x_std : kept-space stats; also *_ordered for reporting
%
% Alexander D. Shaw / 2025
...
X0        = X(:, keep0);
is_one0   = is_onecol(keep0);
orig_idx0 = orig_idx(keep0);

% -------------------------------
% 2) Standardise (current matrix)
% -------------------------------
x_mean0 = mean(X0,1);
x_std0  = std(X0,0,1); x_std0(x_std0==0) = 1; % protect div by 0

if do_std
    Xs = (X0 - x_mean0) ./ x_std0;
    Xs(:, is_one0) = 1;                       % keep exact 1s for intercept
    y_mean = mean(y,1);
...
is_one_std = all(abs(Xs - 1) < 1e-12, 1);
onepos = find(is_one_std(Eqr), 1, 'first');
if ~isempty(onepos), keepQR(onepos) = true; end

% apply selection/permutation
Xs        = Xs(:, Eqr(keepQR));
is_oneQR  = is_one0(Eqr(keepQR));
orig_idxK = orig_idx0(Eqr(keepQR));   % mapping to ORIGINAL columns
x_meanK   = x_mean0(Eqr(keepQR));     % kept-space stats aligned to Xs
x_stdK    = x_std0(Eqr(keepQR));

% neutralise intercept in scaling terms
if any(is_oneQR)
    x_meanK(is_oneQR) = 0;
    x_stdK(is_oneQR)  = 1;
end
...
% -------------------------------
% 5) Initial hyperparams
% -------------------------------
lambda   = ones(p,1);
sigma2   = max(var(ys,0,1), 1e-6);  % 1×d (std y-units)
beta_std = zeros(p,d);
logev    = -inf;

% -------------------------------
% 6) Coordinate ascent (Type-II)
% -------------------------------
for it = 1:max_iter
    lambda_old = lambda;
    sigma2_old = sigma2;
    ...
    if tie_lam
        ...
        [beta_std, diagV] = spd_solve(A, XTy / s2);   % p×d, diag(Σ) entries
    else
        beta_std = zeros(p,d);
        diagV = zeros(p,1);
        for j = 1:d
        ...
        diagV = diagV / d; % average across outputs
    end
...
% -------------------------------
% 7) Final covariance & evidence (std space)
% -------------------------------
s2 = mean(sigma2);
A  = diag(lambda) + (1/s2)*XTX;
...
% -------------------------------
% 8) Unstandardise β to original scale
% -------------------------------
beta = zeros(p,d);
for j = 1:d
    scale = (y_std(j) ./ x_stdK).';   % p×1 (kept-space)
    beta(:,j) = scale .* beta_std(:,j);
end

% implied intercept when standardised and NO explicit intercept kept
intercept = [];
if do_std && ~any(is_oneQR)
    % (1×p)*(p×d) -> (1×d), then scale to original y units
    b0 = y_mean - ((x_meanK ./ x_stdK) * beta_std) .* y_std;
    intercept = b0;   %#ok<NASGU>
end
...
% -------------------------------
% 9) Pack model (kept-space + ordered copies)
% -------------------------------
M.beta          = beta;               % kept order, original y units
M.beta_std      = beta_std;           % kept order, std space
M.Vbeta         = Vbeta_full;         % kept order, std space
M.lambda        = lambda(:);
M.gamma         = gamma(:);
M.sigma2        = sigma2 .* (y_std.^2);   % original y units (per output)
M.logev         = logev;
M.z             = z;
M.pvals         = pvals;

M.x_mean        = x_meanK;            % kept-space stats
M.x_std         = x_stdK;
M.y_mean        = y_mean;
M.y_std         = y_std;

M.kept_columns  = orig_idxK(:);       % map kept -> original
M.used_intercept= any(is_onecol);
M.standardised  = do_std;
M.tie_lambdas   = tie_lam;

% reporting copies in ORIGINAL input order (do NOT overwrite kept-space)
M = peb_reorder_to_original(M, p0);
end
...
function [Xsol, diagInvA, Ainv] = spd_solve(A, B)
...
if flag == 0
    Xsol = R \ (R' \ B);
    Ri   = R \ eye(size(A));
    if nargout >= 2, diagInvA = sum(Ri.^2, 2); end
    if nargout >= 3, Ainv = Ri * Ri.'; end
    return;
end
% SVD SPD-repair
As = (A + A')/2;
[U,S,~] = svd(As,'econ');
s = diag(S); s = max(s, 1e-8);
Ainv = U * bsxfun(@rdivide, U', s');   % inv(A) ≈ U * diag(1/s) * U'
Xsol = Ainv * B;
if nargout >= 2
    diagInvA = sum((U.^2) .* (1./s'), 2);
end
end
...
% orig_idx0 = orig_idx(keep0);
% 
% % standardise, then QR on Xs ...
% [Qqr,Rqr,Eqr] = qr(Xs,0);
% rdiag = abs(diag(Rqr));
% tolQR = max(size(Xs))*eps(max(rdiag));
% keepQR = rdiag > tolQR;
% 
% % ensure intercept kept if present
% is_one_std = all(abs(Xs - 1) < 1e-12, 1);
% onepos = find(is_one_std(Eqr),1,'first');
% if ~isempty(onepos), keepQR(onepos) = true; end
% 
% % apply QR selection/permutation
% Xs        = Xs(:, Eqr(keepQR));
% is_oneQR  = is_one0(Eqr(keepQR));
% orig_idxK = orig_idx0(Eqr(keepQR));  % <-- final mapping to ORIGINAL columns
% % current (kept-space) stats aligned to Xs:
% x_meanK   = x_mean0(Eqr(keepQR));
% x_stdK    = x_std0(Eqr(keepQR));
% 
% 
% % --- precompute
...
% beta = zeros(p,d);
% for j = 1:d
%     scale = (y_std(j) ./ x_stdK).';   % p×1, aligned to kept columns
%     beta(:,j) = scale .* beta_std(:,j);
% end
% 
...
% % end
% 
% % neutralise intercept in scaling terms (so it doesn't distort)
% if any(is_oneQR)
%     x_meanK(is_oneQR) = 0;
%     x_stdK(is_oneQR)  = 1;
% end
% 
% % implied intercept when standardised and NO intercept column kept
% intercept = [];
% if do_std && ~any(is_oneQR)
%     % (1×p)*(p×d) -> (1×d), then scale to original y units
%     b0 = y_mean - ((x_meanK ./ x_stdK) * beta_std) .* y_std;
...
% 
% 
% % kept-space: used by prediction
% M.beta         = beta;           % p×d (kept order)
% M.beta_std     = beta_std;       % p×d (std space, kept order)
% M.Vbeta        = Vbeta_full;     % p×p (std space, kept order)
% M.lambda       = lambda(:);      % p×1
% M.gamma        = gamma(:);       % p×1
% M.x_mean       = x_meanK;        % 1×p
% M.x_std        = x_stdK;         % 1×p
% M.y_mean       = y_mean;         % 1×d
% M.y_std        = y_std;          % 1×d
% M.kept_columns = orig_idxK(:);   % indices into ORIGINAL X
% M.used_intercept = any(is_onecol);
% M.standardised   = do_std;
% M.tie_lambdas    = tie_lam;
% 
% % reporting copies in ORIGINAL input order (do NOT overwrite the kept-space fields)
% M = peb_reorder_to_original(M, p0);
% 
% 
% % % --- pack model
