function [m, V, D, logL, iter, sigma2, allm] = fitVariationalLaplaceThermoRadialPrecision(y, f, m0, S0, maxIter, tol,plots)
...
%
% Returs the posterior: q(z) ~ N(m, V*V' + D)
%
% Key Features:
...
for iter = 1:maxIter
   ...
   % H_prior = inv(S0 + eye(size(S0)) * 1e-6);
    %H_prior = inv(S0 + computeSmoothCovariance(m, 2)); % Instead of just S0
        ...
        % Adaptive smoothing length-scale for radial precision operator
    sigma_schedule = linspace(3.0, 0.3, maxIter); % from smooth to sharper
    sigma_t = sigma_schedule(min(iter, maxIter));
    
    % Radial precision matrix based on current mean estimate
    radial_precision = radialPD(m, sigma_t);
    
    % Updated prior precision using radial structure
    H_prior = inv(S0 + radial_precision);
    H_prior = inv(radial_precision'.*S0.*radial_precision);
